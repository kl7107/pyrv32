...
def test_lb_max_positive_offset(runner):
    ...
    mem.write_byte(0x1000 + 2047, 0x55)
    cpu.write_reg(2, 0x1000)
...
def test_lb_max_negative_offset(runner):
    ...
    mem.write_byte(0x1000 - 2048, 0xAA)
    cpu.write_reg(2, 0x1000)
...
def test_lb_rs1_x0(runner):
    """LB with rs1=x0 loads from address 0 + offset"""
    cpu = RV32CPU()
    mem = Memory()
    
    mem.write_byte(100, 0x66)
    
    # LB x1, 100(x0)
    insn = (100 << 20) | (0 << 15) | (0b000 << 12) | (1 << 7) | 0b0000011
...
def test_lb_multiple_bytes_at_address(runner):
    ...
    # Little-endian: byte at 0x1000 is 0xDD, sign-extends to 0xFFFFFFDD
