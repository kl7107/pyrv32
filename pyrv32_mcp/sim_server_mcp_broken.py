...
class SimulatorMCPServer:
    ...
    async def handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            # Create stdio-like streams from TCP socket
            from mcp.server.stdio import stdio_server
            
            # Wrap the streams - MCP expects text mode
            class StreamWrapper:
                def __init__(self, stream, is_reader=True):
                    self.stream = stream
                ...
                async def read(self, n=-1):
                    if self.is_reader:
                        data = await self.stream.read(n if n > 0 else 8192)
            ...
            stdin_wrapper = StreamWrapper(reader, is_reader=True)
            stdout_wrapper = StreamWrapper(writer, is_reader=False)
            
            # Run the MCP server with wrapped streams
