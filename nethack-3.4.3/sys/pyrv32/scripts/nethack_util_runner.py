...
# Add repository root to path for pyrv32_system imports
...
class UtilityRunner:
    ...
    def _step1_clean_sim_dir(self):
        ...
        # Ensure include directory exists for makedefs header generation
        include_path = repo_root / "pyrv32_sim_fs" / "include"
    ...
    def _step4_run_utility(self, argv_area: int) -> RV32System:
        ...
        # Create envp array at ARGV_AREA + 0x110 (just NULL terminator)
        envp_array = struct.pack('<I', 0)
        for i, byte in enumerate(envp_array):
            sim.memory.write_byte(argv_area + 0x110 + i, byte)
        
        # Set argc=2, argv pointer, envp pointer
        sim.cpu.regs[10] = 2  # a0 = argc
        sim.cpu.regs[11] = argv_area + 0x100  # a1 = argv
        sim.cpu.regs[12] = argv_area + 0x110  # a2 = envp
