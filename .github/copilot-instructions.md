...
6. Only send a final response when a documented hard failure prevents all further progress (cite the file/line, failing MCP tool, reproduction steps, and next debugging action). If no hard failure exists, keep working—do not send a final response, and do not pause.
7. Reserve the word "blocker" exclusively for those documented hard failures; all other work must be described as in-progress requirements, not blockers.
...
- Never leave the workspace with all Immediate Requirements still open; either keep working or document the specific hard failure directly in TODO.md (include failing commands, logs, and your next debugging experiment).
- If no hard failure exists, set at least one Immediate Requirement item to in-progress (with details in TODO.md) before sending any final response so work physically cannot stop between steps.
</RULES>
